...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    // 处理图片
    self.images =
    [_attributedString setImageAlignment:_imageAlignment
...
- (NSMutableAttributedString *)attributedString:(NSString *)text
    ...
    NSRange range =
    [text rangeOfString:[NSString stringWithFormat:@"@%@",@"cccccc"]];
    [attributedString beginEditing];
    [attributedString addAttributes:@{NSForegroundColorAttributeName: HEXCOLOR(0xe69091)} range:NSMakeRange(range.location, range.length)];
...
//                                                 numberOfLines:_numberOfLines];
    ...
    CGSize newSize = [_attributedString boundingSizeForWidth:size.width numberOfLines:_numberOfLines];
...
- (void)drawImages {
    ...
    for (CFIndex index = 0; index < numberOfLines; index++) {
        ...
        for (CFIndex idx = 0; idx < runCount; idx++) {
            ...
            if ([imageData.imageName hasSuffix:@".gif"]) {
                // 绘制图片
                if ([imageData.attributedImage isKindOfClass:[UIImage class]]) {
                    ...
                    imageView.tag = 10000000 * (index + 1);
                    
                    UILabel *label = [[UILabel alloc] init];;
                    label.text = imageData.imageName;
                    label.tag = imageView.tag * 10;
                    [self addSubview:label];
                    
                    if (imageData.imageName.length > 7) {
                ...
                imageView.image = imageData.attributedImage;
                [self addSubview:imageView];
            }
        }
    }
...
{
    UILabel *label = [self viewWithTag: tap.view.tag * 10];
    if (self.attributedLabelDelegate && [self.attributedLabelDelegate respondsToSelector:@selector(attributedLabel:imageName:messageId:)]) {
        [self.attributedLabelDelegate attributedLabel:self imageName:label.text messageId:self.messageId];
    }
}
