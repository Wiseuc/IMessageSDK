...
NSString *const kLinphoneDisplayStatusUpdate                 = @"LinphoneDisplayStatusUpdate";                 //显示状态
NSString *const kLinphoneMessageReceived                     = @"LinphoneMessageReceived";                     //接收到信息
NSString *const kLinphoneTextComposeEvent                    = @"LinphoneTextComposeStarted";                  //开始编辑文字
NSString *const kLinphoneCallUpdate                          = @"LinphoneCallUpdate";                          //通话更新
NSString *const kLinphoneRegistrationUpdate                  = @"LinphoneRegistrationUpdate";                  //注册更新
NSString *const kLinphoneAddressBookUpdate                   = @"LinphoneAddressBookUpdate";                   //通讯录更新
NSString *const kLinphoneMainViewChange                      = @"LinphoneMainViewChange";                      //主视图更新
NSString *const kLinphoneLogsUpdate                          = @"LinphoneLogsUpdate";                          //日志更新
NSString *const kLinphoneSettingsUpdate                      = @"LinphoneSettingsUpdate";                      //设置更新
NSString *const kLinphoneBluetoothAvailabilityUpdate         = @"LinphoneBluetoothAvailabilityUpdate";         //蓝牙可用性更新
NSString *const kLinphoneConfiguringStateUpdate              = @"LinphoneConfiguringStateUpdate";              //配置状态更新
NSString *const kLinphoneGlobalStateUpdate                   = @"LinphoneGlobalStateUpdate";                   //全局状态更新
NSString *const kLinphoneNotifyReceived                      = @"LinphoneNotifyReceived";                      //接收到通知
NSString *const kLinphoneNotifyPresenceReceivedForUriOrTel = @"LinphoneNotifyPresenceReceivedForUriOrTel";     //
NSString *const kLinphoneCallEncryptionChanged               = @"LinphoneCallEncryptionChanged";               //通话加密状态改变
NSString *const kLinphoneFileTransferSendUpdate              = @"LinphoneFileTransferSendUpdate";              //文件传输发送更新
NSString *const kLinphoneFileTransferRecvUpdate              = @"LinphoneFileTransferRecvUpdate";              //文件传输接受更新
            ...
            if(floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_9_x_Max) {
				...
				if (data->notification) {
					...
					if ([[UIDevice currentDevice].systemVersion floatValue] >= 8 &&
						...
						data->notification.soundName = @"notes_of_the_optimistic.caf";
                    ...
                    [NSString stringWithFormat:NSLocalizedString(@"汇讯语音视频", nil), address];
		...
		if (data != nil && data->notification != nil) {
			...
			if (log == NULL || linphone_call_log_get_status(log) == LinphoneCallMissed) {
				...
				notification.alertBody =
					[NSString stringWithFormat:NSLocalizedString(@"您有一个未接电话 %@", nil), address];
