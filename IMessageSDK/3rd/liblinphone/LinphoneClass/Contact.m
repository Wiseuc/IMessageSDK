...
- (instancetype)initWithPerson:(ABRecordRef)aperson andFriend:(LinphoneFriend *)afriend {
	self = [super init];
	_person = aperson;
	_afriend = afriend ? linphone_friend_ref(afriend) : NULL;
	...
	if (_person) {
		...
		_afriend = linphone_friend_list_find_friend_by_ref_key(linphone_core_get_default_friend_list(LC), key);
		if (!_afriend) {
			_afriend = linphone_friend_ref(linphone_core_create_friend(LC));
			linphone_friend_set_ref_key(_afriend, key);
			linphone_friend_set_name(_afriend, [NSString stringWithFormat:@"%@ %@", _firstName, _lastName].UTF8String);
			for (NSString* sipAddr in _sipAddresses) {
				...
				if (addr) {
					...
					linphone_friend_add_address(_afriend, addr);
...
#else
				linphone_friend_add_phone_number(_afriend, phone.UTF8String);
#endif
			}
			if (_afriend) {
				linphone_friend_enable_subscribes(_afriend, FALSE);
				linphone_friend_set_inc_subscribe_policy(_afriend, LinphoneSPDeny);
				linphone_core_add_friend(LC, _afriend);
			}
		}
		linphone_friend_ref(_afriend);
	} else if (_afriend) {
		[self loadFriend];
	} else {
...
- (void)dealloc {
	...
	if (_afriend) {
		linphone_friend_unref(_afriend);
	}
	_person = nil;
	_afriend = NULL;
}
...
- (void)setFirstName:(NSString *)firstName {
	...
	if (_person) {
		...
		ret = (linphone_friend_set_name(_afriend, firstName.UTF8String) == 0);
	}
...
- (BOOL)addSipAddress:(NSString *)sip {
	...
	if (_person) {
		...
		if (addr) {
			...
			ret = (bctbx_list_size(linphone_friend_get_addresses(_afriend)) == _sipAddresses.count + 1);
		}
	}
	if (ret) {
...
- (BOOL)addPhoneNumber:(NSString *)phone {
	...
	if (_person) {
		...
		if (cphone) {
			linphone_friend_add_phone_number(_afriend, cphone);
			phone = [NSString stringWithUTF8String:cphone];
			ms_free(cphone);
			// ensure that it was added by checking list size
			ret = (bctbx_list_size(linphone_friend_get_phone_numbers(_afriend)) == _phoneNumbers.count + 1);
		}
	}
	if (ret) {
...
- (BOOL)removeSipAddressAtIndex:(NSInteger)index {
	...
	if (_person) {
		...
		if (addr) {
			linphone_friend_remove_address(_afriend, addr);
			linphone_address_destroy(addr);
			// ensure that it was destroyed by checking list size
			ret = (bctbx_list_size(linphone_friend_get_addresses(_afriend)) + 1 == _sipAddresses.count);
		}
	}
	if (ret) {
...
- (BOOL)removePhoneNumberAtIndex:(NSInteger)index {
	...
	if (_person) {
		...
		linphone_friend_remove_phone_number(_afriend, phone);
		// ensure that it was destroyed by checking list size
		ret = (bctbx_list_size(linphone_friend_get_phone_numbers(_afriend)) + 1 == _phoneNumbers.count);
	}
	if (ret) {
...
- (void)loadFriend {
	// First and Last name
	{
		_firstName = [NSString stringWithUTF8String:linphone_friend_get_name(_afriend) ?: ""];
		_lastName = nil;
	}
	...
	{
		_phoneNumbers = [[NSMutableArray alloc] init];
		MSList *numbers = linphone_friend_get_phone_numbers(_afriend);
		while (numbers) {
	...
	{
		_sipAddresses = [[NSMutableArray alloc] init];
		const MSList *sips = linphone_friend_get_addresses(_afriend);
		while (sips) {
